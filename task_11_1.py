class Date:
    ...
    def split_date(cls, date):
        try:
            new_date = tuple(map(int, date.split('-'))) if len(
                tuple(map(int, date.split('-')))) == 3 else 'Error'
            if new_date == 'Error':
                raise ValueError
            return new_date
        except ValueError:
...
def tests():
    ...
    assert Date().split_date('89-12-1874-1987') is None
...
def main():
    while True:
        date = input (
            'Введите дату в формате дд-мм-гг, "q" для выхода или "t" для тестирования: ' )
        if date == 'q':
            break
        elif date == 't':
            tests ()
